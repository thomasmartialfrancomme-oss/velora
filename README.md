...
3. **Database.** Default path `./data/velora.db`; set `VELORA_DB_PATH` to put it on a persistent volume. `npm run db:prepare` (`scripts/prepare-runtime.mjs`) is what a fresh host needs: it creates `data/` and `data/uploads/`, applies `db/schema.sql`, and seeds the demonstration households **only** when `VELORA_SEED_DEMO=1` — those passphrases are published in this repository, so a deployment reachable from the internet should not start with them. `npm run db:seed` is the manual equivalent. The same rule governs the process itself: on an empty database a production boot applies the schema and stops there unless `VELORA_SEED_DEMO=1` is set, while development still seeds on first open so `npm run dev` opens on a populated household. A real signup is never left with an empty ledger — `seedStarterData` (`src/lib/auth/session.ts`) gives every new account a starter residence, six expense categories and an opening task. Back up with `sqlite3 data/velora.db ".backup …"` — WAL, so a bare file copy while running is not enough. On Render the disk is snapshotted every 24 h, but Render's own guidance is not to restore a disk that backs a database, so treat that snapshot as disaster cover for the uploads and restore SQLite from your `.backup` copy.
