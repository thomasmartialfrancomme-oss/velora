      ...
      <div className="container py-16 sm:py-20">
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1.3fr)_repeat(3,minmax(0,0.6fr))_auto]">
          <div className="max-w-sm">
            <span className="inline-flex items-center gap-3">
              <VeloraMark size={26} />
              <span className="font-sans text-[15px] uppercase tracking-[0.4em] text-ivory-50">Velora</span>
            </span>
            <p className="mt-5 font-serif text-[1.15rem] leading-relaxed text-graphite-200">
              Private intelligence for modern estates and lifestyles.
            </p>
            <p className="mt-4 text-[12px] leading-relaxed text-graphite-500">
              Members only. Available in selected jurisdictions, by introduction or application.
            </p>
          </div>

          {COLUMNS.map((column) => (
            <nav key={column.title} aria-label={column.title}>
              <p className="label mb-5 text-graphite-400">{column.title}</p>
          ...
          <div className="lg:justify-self-end">
            <p className="label mb-5 text-graphite-400">Begin</p>
            <Button href="/access/request" variant="gold-outline" size="md" asLink>
              Request private access
            </Button>
            <p className="mt-4 max-w-[190px] text-[11.5px] leading-relaxed text-graphite-500">
              Applications are reviewed personally. Expect a reply within two business days.
            </p>
          </div>
        </div>
        ...
        <div className="mt-16 flex flex-col gap-4 border-t border-ivory-200/[0.06] pt-8 text-[11px] text-graphite-500 sm:flex-row sm:items-center sm:justify-between">
          ...
          <p className="tracking-[0.1em]">
            VELORA organises information and records. It does not provide legal, tax or financial advice.
          </p>
        </div>
      </div>
    </footer>
