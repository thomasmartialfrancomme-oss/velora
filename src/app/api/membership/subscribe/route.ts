...
export const POST = api({
  ...
  handler: async ({ user, body }) => {
    const payload = body as { plan: 'private' | 'priority' | 'private_office'; billingCycle: 'monthly' | 'annual' };
    const plan = planFor(payload.plan);
    if (!plan) return fail(422, 'unknown_plan', 'That plan is not offered.');

    const intent = await getBillingProvider().checkout({
    ...
    audit({ userId: user.id, event: 'membership.checkout_started', target: plan.key, meta: { provider: intent.provider, simulated: intent.simulated } });
